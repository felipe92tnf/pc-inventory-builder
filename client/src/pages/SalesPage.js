...
import { combinePcMonthWithServices, extendYearRangeWithServices, filterSalesByMonth, isActiveSale, averageMonthlySalaryBefore, mergeSalesAndServicesMonthlySummaries, mergeYearTotalsFromMonthlySummaries, minMaxYearsFromData, monthlyProfitSeriesCombined, monthTotalsFromSales, marginPercentOnRevenue, pctDelta, prevMonthYear, rankBuildsByProfit, rankClientsBySpend, yearTotalsFromSummary } from "../utils/salesStats";
...
function YearMonthlyProfitChart({ series, year }) {
    ...
    return (_jsx("div", { className: "overflow-x-auto pb-2", children: _jsx("div", { className: "flex min-w-[640px] items-end gap-2 md:gap-3", style: { height: chartRowHeight }, children: series.map(({ month, profit }) => {
                const barH = allZero
                    ? 0
                    : Math.max(((profit - minV) / span) * CHART_BAR_MAX_PX, Math.abs(profit) > 1e-6 ? 3 : 0);
                return (_jsxs("div", { className: "flex min-w-0 flex-1 flex-col items-center justify-end", children: [_jsx("span", { className: "text-center text-[10px] font-medium capitalize leading-tight text-slate-500", children: shortMonth(month, year) }), _jsx("span", { className: `mt-0.5 text-center text-[10px] font-semibold tabular-nums leading-tight ${profit >= 0 ? "text-emerald-300/95" : "text-rose-300/95"}`, children: money(profit) }), _jsx("div", { className: `mt-1 w-full max-w-full rounded-t-md shadow-sm transition hover:opacity-90 ${profit >= 0
                                ? "bg-gradient-to-t from-indigo-700 to-emerald-500/75 shadow-indigo-900/40"
                                : "bg-gradient-to-t from-rose-900 to-rose-500/80 shadow-rose-950/40"}`, style: { height: barH }, title: `${shortMonth(month, year)}: ${money(profit)}` })] }, month));
            }) }) }));
}
export function SalesPage() {
    ...
    const averageSalaryBeforeSelected = useMemo(() => averageMonthlySalaryBefore(mergedHistoricSummary, selectedYear, selectedMonth), [mergedHistoricSummary, selectedYear, selectedMonth]);
    const salesInSelectedMonth = useMemo(() => filterSalesByMonth(sales, selectedYear, selectedMonth), [sales, selectedYear, selectedMonth]);
    const deliveredPcSalesInMonth = useMemo(() => salesInSelectedMonth.filter((s) => isActiveSale(s) && s.pickupConfirmedAt != null), [salesInSelectedMonth]);
    const pendingPickupPcSalesInMonth = useMemo(() => salesInSelectedMonth.filter((s) => isActiveSale(s) && s.pickupConfirmedAt == null), [salesInSelectedMonth]);
    const revertedPcSalesInMonth = useMemo(() => salesInSelectedMonth.filter((s) => s.status === "REVERTED"), [salesInSelectedMonth]);
    useEffect(() => {
        let active = true;
