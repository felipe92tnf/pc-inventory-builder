...
import { StatusBadge, collectionPaymentVariant, serviceStatusVariant } from "../components/ui/StatusBadge";
...
import { PaymentCollectionSection } from "../components/payments/PaymentCollectionSection";
...
export function ServicesPage() {
    ...
    const { services, loading, error, submitting, actionId, reload, createService, patchService, registerServicePayment, deleteService, completeService } = useServices(filterMonth, filterYear, typeParam, statusParam);
    ...
    const [initialPaidAtCreate, setInitialPaidAtCreate] = useState("");
    ...
    const handleSubmit = async (event) => {
        ...
        const paidTrim = initialPaidAtCreate.trim();
        let initialPaidAmount;
        if (paidTrim !== "") {
            const p = Number(paidTrim.replace(",", "."));
            if (!Number.isFinite(p) || p < 0) {
                window.alert("Importe cobrado invalido o deja el campo vacio (cobro completo).");
                return;
            }
            initialPaidAmount = Math.round(p * 100) / 100;
        }
        const initialOpt = initialPaidAmount !== undefined ? { initialPaidAmount } : {};
        try {
            if (formType === "SPARE_PART_SALE") {
...
function CompletedSubsection({ label, accent, rows, actionId, onComplete, onCancel, onDelete, onEditService, onOpenPayment }) {
    const st = aggregateStats(rows);
    return (_jsxs("div", { className: `rounded-lg border ${accent} p-4`, children: [_jsxs("div", { className: "mb-3 flex flex-wrap items-center justify-between gap-2", children: [_jsx("h3", { className: "text-sm font-semibold uppercase tracking-wide text-slate-300", children: label }), _jsx(StatusBadge, { variant: "neutral", size: "table", className: "tabular-nums", children: st.count })] }), _jsx(ServiceListSection, { rows: rows, actionId: actionId, completedActions: true, onEditService: onEditService, onOpenPayment: onOpenPayment, onComplete: onComplete, onCancel: onCancel, onDelete: onDelete })] }));
}
...
function ServiceListSection({ rows, actionId, completedActions = false, onEditService, onOpenPayment, onComplete, onCancel, onDelete }) {
    if (rows.length === 0)
        return null;
    return (_jsxs(_Fragment, { children: [_jsx("div", { className: "hidden overflow-hidden rounded-lg border border-slate-800 bg-slate-900/50 md:block", children: _jsx("div", { className: "overflow-x-auto", children: _jsxs("table", { className: "w-full min-w-[720px] text-left text-xs text-slate-200", children: [_jsx("thead", { className: "bg-slate-950/80 text-[10px] uppercase tracking-wide text-slate-500", children: _jsxs("tr", { children: [_jsx("th", { className: "px-2 py-2", children: "Fecha" }), _jsx("th", { className: "px-2 py-2", children: "Titulo" }), _jsx("th", { className: "px-2 py-2", children: "Tipo" }), _jsx("th", { className: "px-2 py-2", children: "Cliente" }), _jsx("th", { className: "px-2 py-2", children: "Estado" }), _jsx("th", { className: "px-2 py-2", children: "Cobro" }), _jsx("th", { className: "px-2 py-2", children: "Venta" }), _jsx("th", { className: "px-2 py-2", children: "Beneficio" }), _jsx("th", { className: "px-2 py-2 text-right", children: "Acciones" })] }) }), _jsx("tbody", { className: "divide-y divide-slate-800", children: rows.map((s) => (_jsx(ServiceTableRow, { s: s, actionId: actionId, completedActions: completedActions, onEditService: onEditService, onOpenPayment: onOpenPayment, onComplete: onComplete, onCancel: onCancel, onDelete: onDelete }, s.id))) })] }) }) }), _jsx("div", { className: "space-y-2 md:hidden", children: rows.map((s) => (_jsx(ServiceCard, { s: s, actionId: actionId, completedActions: completedActions, onEditService: onEditService, onOpenPayment: onOpenPayment, onComplete: onComplete, onCancel: onCancel, onDelete: onDelete }, s.id))) })] }));
}
function ServiceTableRow({ s, actionId, completedActions = false, onEditService, onOpenPayment, onComplete, onCancel, onDelete }) {
    const d = new Date(s.serviceDate);
    const dateStr = d.toLocaleDateString("es-ES");
    const spareHint = spareSaleSummary(s);
    return (_jsxs("tr", { className: "transition hover:bg-slate-800/40", children: [_jsx("td", { className: "whitespace-nowrap px-2 py-2 text-slate-400", children: dateStr }), _jsxs("td", { className: "max-w-[200px] px-2 py-2", children: [_jsx("div", { className: "truncate font-medium text-slate-100", title: s.title, children: s.title }), spareHint ? (_jsx("div", { className: "truncate text-[10px] text-slate-500", title: spareHint, children: spareHint })) : null] }), _jsx("td", { className: "max-w-[100px] truncate px-2 py-2 text-[11px] text-slate-400", title: SERVICE_LABELS[s.type], children: SERVICE_LABELS[s.type] }), _jsxs("td", { className: "max-w-[120px] px-2 py-2 text-slate-300", children: [_jsx("div", { className: "truncate font-medium", title: s.customerName, children: s.customerName }), _jsx(CustomerProfileLink, { customerName: s.customerName, customerPhone: s.customerPhone, className: "mt-0.5 inline-flex text-[10px]" })] }), _jsx("td", { className: "px-2 py-2", children: _jsx(StatusBadge, { variant: serviceStatusVariant(s.status), size: "table", children: STATUS_LABELS[s.status] }) }), _jsx("td", { className: "px-2 py-2", children: _jsxs("div", { className: "flex flex-col gap-1", children: [_jsx(StatusBadge, { variant: collectionPaymentVariant(s.paymentStatus), size: "table", children: COLLECTION_PAYMENT_LABELS[s.paymentStatus] }), s.remainingAmount > 0.005 ? (_jsxs("span", { className: "text-[10px] text-amber-200/90", children: ["Pend. ", money(s.remainingAmount)] })) : null] }) }), _jsx("td", { className: "whitespace-nowrap px-2 py-2 text-slate-300", children: money(s.salePrice) }), _jsx("td", { className: "whitespace-nowrap px-2 py-2 text-emerald-300/90", children: money(s.profit) }), _jsx("td", { className: "px-2 py-2", children: _jsxs("div", { className: "flex flex-wrap justify-end gap-1", children: [s.status === "PENDING" ? (_jsxs(_Fragment, { children: [_jsx("button", { type: "button", disabled: actionId === s.id, onClick: () => onComplete(s.id), className: PRIMARY_ACTION_BUTTON_COMPACT, children: "Completar" }), _jsx("button", { type: "button", disabled: actionId === s.id, onClick: () => onCancel(s.id), className: SECONDARY_BUTTON_SM, children: "Cancelar" })] })) : null, s.remainingAmount > 0.005 ? (_jsx("button", { type: "button", disabled: actionId === s.id, onClick: () => onOpenPayment(s), className: SECONDARY_BUTTON_SM, children: "Registrar pago" })) : null, completedActions && s.status === "COMPLETED" && onEditService ? (_jsx("button", { type: "button", disabled: actionId === s.id, onClick: () => onEditService(s), className: ORANGE_EDIT_BUTTON_SM, children: "Editar" })) : (_jsx("button", { type: "button", disabled: actionId === s.id, onClick: () => onDelete(s.id), className: DESTRUCTIVE_BUTTON_SM, children: "Eliminar" }))] }) })] }));
}
function ServiceCard({ s, actionId, completedActions = false, onEditService, onOpenPayment, onComplete, onCancel, onDelete }) {
    const d = new Date(s.serviceDate);
    const spareHint = spareSaleSummary(s);
    return (_jsxs("article", { className: "rounded-xl border border-slate-800 bg-slate-950/60 p-3 shadow-sm", children: [_jsxs("div", { className: "flex flex-wrap items-start justify-between gap-2", children: [_jsxs("div", { className: "min-w-0 flex-1", children: [_jsx("p", { className: "line-clamp-2 break-words font-semibold text-slate-100", children: s.title }), spareHint ? (_jsx("p", { className: "truncate text-[11px] text-slate-500", title: spareHint, children: spareHint })) : null, _jsx("p", { className: "text-[11px] text-slate-500", children: d.toLocaleDateString("es-ES") }), _jsx("p", { className: "mt-0.5 truncate text-[11px] text-slate-400", title: SERVICE_LABELS[s.type], children: SERVICE_LABELS[s.type] })] }), _jsxs("div", { className: "flex flex-shrink-0 flex-col items-end gap-1", children: [_jsx(StatusBadge, { variant: serviceStatusVariant(s.status), size: "table", children: STATUS_LABELS[s.status] }), _jsx(StatusBadge, { variant: collectionPaymentVariant(s.paymentStatus), size: "table", children: COLLECTION_PAYMENT_LABELS[s.paymentStatus] })] })] }), _jsx("p", { className: "truncate text-sm text-slate-300", children: s.customerName }), _jsx(CustomerProfileLink, { customerName: s.customerName, customerPhone: s.customerPhone, className: "mt-1 inline-flex text-xs" }), _jsxs("dl", { className: "mt-3 space-y-1.5 border-t border-slate-800 pt-3 text-sm", children: [_jsxs("div", { className: "flex justify-between gap-3", children: [_jsx("dt", { className: "shrink-0 text-xs text-slate-500", children: "Coste" }), _jsx("dd", { className: "min-w-0 text-right font-medium text-slate-300", children: money(s.costPrice) })] }), _jsxs("div", { className: "flex justify-between gap-3", children: [_jsx("dt", { className: "shrink-0 text-xs text-slate-500", children: "Venta" }), _jsx("dd", { className: "min-w-0 text-right text-base font-semibold text-emerald-300", children: money(s.salePrice) })] }), _jsxs("div", { className: "flex justify-between gap-3", children: [_jsx("dt", { className: "shrink-0 text-xs text-slate-500", children: "Beneficio" }), _jsx("dd", { className: "min-w-0 text-right text-base font-semibold text-emerald-300", children: money(s.profit) })] }), s.remainingAmount > 0.005 ? (_jsxs("div", { className: "flex justify-between gap-3", children: [_jsx("dt", { className: "shrink-0 text-xs text-slate-500", children: "Pendiente cobro" }), _jsx("dd", { className: "min-w-0 text-right text-sm font-semibold text-amber-200", children: money(s.remainingAmount) })] })) : null] }), _jsxs("div", { className: "mt-3 flex flex-col gap-2 border-t border-slate-800 pt-3", children: [s.remainingAmount > 0.005 ? (_jsx("button", { type: "button", disabled: actionId === s.id, onClick: () => onOpenPayment(s), className: `${SECONDARY_BUTTON} ${SERVICE_CARD_ACTION_TOUCH}`, children: "Registrar pago" })) : null, s.status === "PENDING" ? (_jsxs(_Fragment, { children: [_jsx("button", { type: "button", disabled: actionId === s.id, onClick: () => onComplete(s.id), className: `${PRIMARY_ACTION_BUTTON} text-sm`, children: "Completar" }), _jsx("button", { type: "button", disabled: actionId === s.id, onClick: () => onCancel(s.id), className: `${SECONDARY_BUTTON} ${SERVICE_CARD_ACTION_TOUCH}`, children: "Cancelar" })] })) : null, completedActions && s.status === "COMPLETED" && onEditService ? (_jsx("button", { type: "button", disabled: actionId === s.id, onClick: () => onEditService(s), className: ORANGE_EDIT_BUTTON_CARD, children: "Editar" })) : (_jsx("button", { type: "button", disabled: actionId === s.id, onClick: () => onDelete(s.id), className: `${DESTRUCTIVE_BUTTON_SM} ${SERVICE_CARD_ACTION_TOUCH}`, children: "Eliminar" }))] })] }));
}
