...
import { PRIMARY_ACTION_BUTTON, PRIMARY_ACTION_BUTTON_COMPACT, SECONDARY_BUTTON_SM, FILTER_TOGGLE_ROW, DESTRUCTIVE_BUTTON_SM } from "../theme/actionButtons";
import { SUMMARY_CARD_GRID, SUMMARY_CARD_LABEL, SUMMARY_CARD_SHELL, SUMMARY_VALUE_NEGATIVE, SUMMARY_VALUE_NEUTRAL, SUMMARY_VALUE_PROFIT_CYAN, SUMMARY_VALUE_REVENUE } from "../theme/summaryCards";
...
function StatusAccordion({ title, tone, open, onToggle, stats, emptyHint, children }) {
    ...
    return (_jsxs("div", { className: `rounded-xl border ${toneBorder} bg-slate-950/40`, children: [_jsxs("button", { type: "button", className: "flex w-full flex-wrap items-center justify-between gap-2 px-3 py-3 text-left sm:px-4", onClick: onToggle, "aria-expanded": open, children: [_jsxs("div", { className: "flex min-w-0 flex-1 flex-wrap items-center gap-2 sm:gap-3", children: [_jsx("span", { className: "text-lg font-semibold text-slate-100", children: title }), _jsx("span", { className: `inline-flex shrink-0 rounded-full border px-2.5 py-0.5 text-xs font-semibold ${toneBadge}`, children: stats.count })] }), _jsx(ChevronDown, { open: open })] }), open ? (_jsx("div", { className: "border-t border-slate-800/90 px-3 pb-3 pt-1 sm:px-4", children: stats.count === 0 ? (_jsx("p", { className: "py-3 text-sm text-slate-500", children: emptyHint })) : (children) })) : null] }));
}
function CompletedSubsection({ label, accent, rows, actionId, onComplete, onCancel, onDelete }) {
    const st = aggregateStats(rows);
    return (_jsxs("div", { className: `rounded-lg border ${accent} p-4`, children: [_jsxs("div", { className: "mb-3 flex flex-wrap items-center justify-between gap-2", children: [_jsx("h3", { className: "text-sm font-semibold uppercase tracking-wide text-slate-300", children: label }), _jsx("span", { className: "rounded-full border border-slate-600 bg-slate-900/80 px-2 py-0.5 text-xs font-semibold text-slate-300", children: st.count })] }), _jsx(ServiceListSection, { rows: rows, actionId: actionId, onComplete: onComplete, onCancel: onCancel, onDelete: onDelete })] }));
}
function ServiceListSection({ rows, actionId, onComplete, onCancel, onDelete }) {
    if (rows.length === 0)
        return null;
    return (_jsxs(_Fragment, { children: [_jsx("div", { className: "hidden overflow-hidden rounded-lg border border-slate-800 bg-slate-900/50 md:block", children: _jsx("div", { className: "overflow-x-auto", children: _jsxs("table", { className: "w-full min-w-[720px] text-left text-xs text-slate-200", children: [_jsx("thead", { className: "bg-slate-950/80 text-[10px] uppercase tracking-wide text-slate-500", children: _jsxs("tr", { children: [_jsx("th", { className: "px-2 py-2", children: "Fecha" }), _jsx("th", { className: "px-2 py-2", children: "Titulo" }), _jsx("th", { className: "px-2 py-2", children: "Tipo" }), _jsx("th", { className: "px-2 py-2", children: "Cliente" }), _jsx("th", { className: "px-2 py-2", children: "Estado" }), _jsx("th", { className: "px-2 py-2", children: "Venta" }), _jsx("th", { className: "px-2 py-2", children: "Beneficio" }), _jsx("th", { className: "px-2 py-2 text-right", children: "Acciones" })] }) }), _jsx("tbody", { className: "divide-y divide-slate-800", children: rows.map((s) => (_jsx(ServiceTableRow, { s: s, actionId: actionId, onComplete: onComplete, onCancel: onCancel, onDelete: onDelete }, s.id))) })] }) }) }), _jsx("div", { className: "space-y-2 md:hidden", children: rows.map((s) => (_jsx(ServiceCard, { s: s, actionId: actionId, onComplete: onComplete, onCancel: onCancel, onDelete: onDelete }, s.id))) })] }));
}
function ServiceTableRow({ s, actionId, onComplete, onCancel, onDelete }) {
    const d = new Date(s.serviceDate);
    const dateStr = d.toLocaleDateString("es-ES");
    const spareHint = spareSaleSummary(s);
    return (_jsxs("tr", { className: "transition hover:bg-slate-800/40", children: [_jsx("td", { className: "whitespace-nowrap px-2 py-2 text-slate-400", children: dateStr }), _jsxs("td", { className: "max-w-[200px] px-2 py-2", children: [_jsx("div", { className: "truncate font-medium text-slate-100", title: s.title, children: s.title }), spareHint ? (_jsx("div", { className: "truncate text-[10px] text-slate-500", title: spareHint, children: spareHint })) : null] }), _jsx("td", { className: "max-w-[100px] truncate px-2 py-2 text-[11px] text-slate-400", title: SERVICE_LABELS[s.type], children: SERVICE_LABELS[s.type] }), _jsx("td", { className: "max-w-[100px] truncate px-2 py-2 text-slate-300", title: s.customerName, children: s.customerName }), _jsx("td", { className: "px-2 py-2", children: _jsx("span", { className: `rounded-full border px-1.5 py-0.5 text-[10px] font-semibold ${s.status === "COMPLETED"
                        ? "border-emerald-500/40 bg-emerald-500/15 text-emerald-300"
                        : s.status === "PENDING"
                            ? "border-amber-500/40 bg-amber-500/15 text-amber-300"
                            : "border-slate-600 bg-slate-800 text-slate-400"}`, children: STATUS_LABELS[s.status] }) }), _jsx("td", { className: "whitespace-nowrap px-2 py-2 text-slate-300", children: money(s.salePrice) }), _jsx("td", { className: "whitespace-nowrap px-2 py-2 text-emerald-300/90", children: money(s.profit) }), _jsx("td", { className: "px-2 py-2", children: _jsxs("div", { className: "flex flex-wrap justify-end gap-1", children: [s.status === "PENDING" ? (_jsxs(_Fragment, { children: [_jsx("button", { type: "button", disabled: actionId === s.id, onClick: () => onComplete(s.id), className: PRIMARY_ACTION_BUTTON_COMPACT, children: "Completar" }), _jsx("button", { type: "button", disabled: actionId === s.id, onClick: () => onCancel(s.id), className: SECONDARY_BUTTON_SM, children: "Cancelar" })] })) : null, _jsx("button", { type: "button", disabled: actionId === s.id, onClick: () => onDelete(s.id), className: DESTRUCTIVE_BUTTON_SM, children: "Eliminar" })] }) })] }));
}
function ServiceCard({ s, actionId, onComplete, onCancel, onDelete }) {
    const d = new Date(s.serviceDate);
    const spareHint = spareSaleSummary(s);
    return (_jsxs("article", { className: "rounded-xl border border-slate-800 bg-slate-950/60 p-3 shadow-sm", children: [_jsxs("div", { className: "flex flex-wrap items-start justify-between gap-2", children: [_jsxs("div", { className: "min-w-0 flex-1", children: [_jsx("p", { className: "truncate font-semibold text-slate-100", children: s.title }), spareHint ? (_jsx("p", { className: "truncate text-[11px] text-slate-500", title: spareHint, children: spareHint })) : null, _jsx("p", { className: "text-[11px] text-slate-500", children: d.toLocaleDateString("es-ES") })] }), _jsx("span", { className: `shrink-0 rounded-full border px-2 py-0.5 text-[10px] font-semibold ${s.status === "COMPLETED"
                            ? "border-emerald-500/40 bg-emerald-500/15 text-emerald-300"
                            : s.status === "PENDING"
                                ? "border-amber-500/40 bg-amber-500/15 text-amber-300"
                                : "border-slate-600 bg-slate-800 text-slate-400"}`, children: STATUS_LABELS[s.status] })] }), _jsx("p", { className: "truncate text-sm text-slate-300", children: s.customerName }), _jsxs("dl", { className: "mt-3 grid grid-cols-2 gap-2 border-t border-slate-800 pt-3 text-sm", children: [_jsxs("div", { children: [_jsx("dt", { className: "text-xs text-slate-500", children: "Venta" }), _jsx("dd", { className: "text-base font-semibold text-slate-200", children: money(s.salePrice) })] }), _jsxs("div", { children: [_jsx("dt", { className: "text-xs text-slate-500", children: "Beneficio" }), _jsx("dd", { className: "text-base font-semibold text-emerald-300", children: money(s.profit) })] })] }), _jsxs("div", { className: "mt-2 flex flex-wrap gap-2", children: [s.status === "PENDING" ? (_jsxs(_Fragment, { children: [_jsx("button", { type: "button", disabled: actionId === s.id, onClick: () => onComplete(s.id), className: PRIMARY_ACTION_BUTTON_COMPACT, children: "Completar" }), _jsx("button", { type: "button", disabled: actionId === s.id, onClick: () => onCancel(s.id), className: SECONDARY_BUTTON_SM, children: "Cancelar" })] })) : null, _jsx("button", { type: "button", disabled: actionId === s.id, onClick: () => onDelete(s.id), className: DESTRUCTIVE_BUTTON_SM, children: "Eliminar" })] })] }));
}
