...
export function RegisterSaleForm({ buildId, suggestedSalePrice, disabled, onSuccess, variant = "card", submitLabel }) {
    ...
    const handleSubmit = async (event) => {
        ...
        const paidRaw = initialPaid.trim();
        if (paidRaw !== "") {
            const paidNorm = Number(paidRaw.replace(",", ".").trim());
            if (!Number.isFinite(paidNorm) || paidNorm < 0) {
                window.alert("Introduce un importe cobrado valido o dejalo vacio (cobro completo).");
                return;
            }
            payload.initialPaidAmount = Math.round(paidNorm * 100) / 100;
        }
        setSubmitting(true);
        try {
