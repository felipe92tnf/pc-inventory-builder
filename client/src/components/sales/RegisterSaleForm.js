...
export function RegisterSaleForm({ buildId, suggestedSalePrice, disabled, onSuccess, variant = "card", submitLabel, offerPendingPickup = false, defaultCustomer }) {
    const [customerName, setCustomerName] = useState(() => defaultCustomer?.customerName?.trim() ?? "");
    const [customerPhone, setCustomerPhone] = useState(() => defaultCustomer?.customerPhone?.trim() ?? "");
    const [customerEmail, setCustomerEmail] = useState(() => defaultCustomer?.customerEmail?.trim() ?? "");
