...
export function BuildItemsTable({ items, status, actionLoading, onRemove, onUpdateLineSale, prominent = false, compactDensity = false }) {
    if (items.length === 0) {
        return (_jsx("section", { className: "rounded-xl border border-slate-800/90 bg-slate-900/60 px-3 py-2.5", children: _jsx("p", { className: "text-xs text-slate-400", children: "Sin piezas en el montaje." }) }));
    }
...
function BuildItemRow({ item, editableSale, actionLoading, status, prominent, compactDensity = false, onRemove, onUpdateLineSale }) {
    ...
    return (_jsxs("tr", { className: "transition hover:bg-slate-800/50", children: [_jsx("td", { className: partCell, children: item.part.name }), _jsx("td", { className: qtyCell, children: item.quantity }), _jsx("td", { className: moneyCell, children: money(item.unitCost) }), _jsx("td", { className: tdBase, children: editableSale && onUpdateLineSale ? (_jsx("div", { className: "flex min-w-[12rem] flex-col gap-1 sm:min-w-[14rem]", children: _jsxs("div", { className: "flex flex-wrap items-center gap-2", children: [_jsx("input", { type: "text", inputMode: "decimal", defaultValue: lineSale.toFixed(2), disabled: actionLoading, onBlur: (event) => {
                    ...
                    }, disabled: status !== "DRAFT" || actionLoading, className: "rounded-md border border-rose-500/40 bg-rose-500/10 px-2.5 py-1 text-[11px] font-semibold text-rose-200 transition hover:bg-rose-500/20 disabled:cursor-not-allowed disabled:opacity-50", children: "Quitar" }) })] }));
}
