...
function isBoilerplateNoteLine(line, build) {
    ...
    if (colonIdx > 0 && colonIdx < t.length - 1) {
        ...
        if ((key === "cliente" || key === "nombre") && name && valCompact === name)
            return true;
        if ((key === "teléfono" || key === "telefono") && phone && valCompact === phone)
            return true;
