...
export function ServiceConceptLinesSection({ lines, onLinesChange, servicePresets, isHomeService, onHomeServiceChange, disabled = false }) {
    ...
    return (_jsxs("div", { className: "space-y-4", children: [_jsxs("label", { className: "flex cursor-pointer items-center gap-3 rounded-xl border border-violet-500/30 bg-violet-950/20 px-4 py-3", children: [_jsx("input", { type: "checkbox", checked: isHomeService, onChange: (e) => handleHomeToggle(e.target.checked), disabled: disabled, className: "h-4 w-4 rounded border-slate-600 bg-slate-950 text-violet-500" }), _jsx("span", { className: "text-sm text-slate-200", children: "Servicio a domicilio \u2014 a\u00F1ade concepto de 20 EUR (editable en la tabla)" })] }), _jsx(CatalogBlock, { presetPickId: presetPickId, setPresetPickId: setPresetPickId, catalogQuery: catalogQuery, setCatalogQuery: setCatalogQuery, filteredPresets: filteredPresets, servicePresets: servicePresets, disabled: disabled, onAddPreset: (preset) => {
                    addPreset(preset);
                    setPresetPickId("");
                } }), _jsxs("div", { className: "rounded-xl border border-slate-800 bg-slate-950/40", children: [_jsxs("button", { type: "button", disabled: disabled, onClick: () => setManualOpen((v) => !v), className: "flex w-full items-center justify-between px-4 py-3 text-left text-sm font-semibold text-slate-200", children: ["+ A\u00F1adir concepto manual", _jsx("span", { className: "text-slate-500", children: manualOpen ? "▲" : "▼" })] }), manualOpen ? (_jsx("div", { className: "border-t border-slate-800 px-4 pb-4 pt-3", children: _jsx(ManualConceptGrid, { manualName: manualName, setManualName: setManualName, manualQty: manualQty, setManualQty: setManualQty, manualCost: manualCost, setManualCost: setManualCost, manualSale: manualSale, setManualSale: setManualSale, disabled: disabled, onAdd: addManualLine }) })) : null] }), _jsxs("section", { className: "overflow-hidden rounded-2xl border border-slate-800 bg-slate-900/80 shadow-lg shadow-slate-950/40", children: [_jsxs("div", { className: "border-b border-slate-800 px-3 py-2 md:px-4", children: [_jsx("h3", { className: "text-sm font-semibold text-slate-100", children: "Conceptos del servicio" }), _jsx("p", { className: "mt-0.5 text-xs text-slate-500", children: "Edita cantidades y precios; el total se calcula autom\u00E1ticamente." })] }), _jsx("div", { className: "overflow-x-auto", children: _jsxs("table", { className: "min-w-full text-left text-sm text-slate-200", children: [_jsx("thead", { className: "bg-slate-950/70 text-xs uppercase tracking-wide text-slate-400", children: _jsxs("tr", { children: [_jsx("th", { className: TABLE_CELL, children: "Concepto" }), _jsx("th", { className: TABLE_CELL, children: "Cant." }), _jsx("th", { className: TABLE_CELL, children: "Coste u." }), _jsx("th", { className: TABLE_CELL, children: "Venta u." }), _jsx("th", { className: `${TABLE_CELL} text-right`, children: "Total" }), _jsx("th", { className: `${TABLE_CELL} text-right`, children: "Acciones" })] }) }), _jsx("tbody", { className: "divide-y divide-slate-800", children: lines.map((line) => (_jsxs("tr", { className: isHomeDeliveryLine(line) ? "bg-violet-950/25" : "hover:bg-slate-800/40", children: [_jsx("td", { className: TABLE_CELL, children: _jsx("input", { value: line.name, disabled: disabled || isHomeDeliveryLine(line), onChange: (e) => updateLine(line.clientKey, { name: e.target.value }), className: `${INPUT} min-w-[10rem]`, placeholder: "Concepto" }) }), _jsx("td", { className: TABLE_CELL, children: _jsx("input", { type: "number", min: 1, value: line.quantity, disabled: disabled, onChange: (e) => updateLine(line.clientKey, {
...
function CatalogBlock({ presetPickId, setPresetPickId, catalogQuery, setCatalogQuery, filteredPresets, servicePresets, disabled, onAddPreset }) {
    return (_jsxs("div", { className: "rounded-xl border border-indigo-500/25 bg-indigo-950/20 p-4", children: [_jsx("p", { className: "text-xs font-semibold uppercase tracking-wide text-indigo-200/80", children: "Cat\u00E1logo de servicios" }), _jsxs("div", { className: "mt-3 flex flex-col gap-2 sm:flex-row sm:flex-wrap sm:items-end", children: [_jsxs("label", { className: "flex min-w-[10rem] flex-1 flex-col gap-1 text-xs font-medium text-slate-400", children: ["Buscar", _jsx("input", { value: catalogQuery, onChange: (e) => setCatalogQuery(e.target.value), disabled: disabled, placeholder: "Filtrar\u2026", className: INPUT })] }), _jsxs("label", { className: "flex min-w-[12rem] flex-1 flex-col gap-1 text-xs font-medium text-slate-400", children: ["Servicio", _jsxs("select", { value: presetPickId, onChange: (e) => setPresetPickId(e.target.value), disabled: disabled || servicePresets.length === 0, className: INPUT, children: [_jsx("option", { value: "", children: servicePresets.length === 0 ? "Sin catálogo" : "Elegir servicio…" }), filteredPresets.map((t) => (_jsxs("option", { value: t.id, children: [t.name, " \u2014 ", Number(t.defaultSalePrice).toFixed(2), " EUR"] }, t.id)))] })] }), _jsx("button", { type: "button", disabled: disabled || !presetPickId, className: PRIMARY_ACTION_BUTTON_COMPACT, onClick: () => {
