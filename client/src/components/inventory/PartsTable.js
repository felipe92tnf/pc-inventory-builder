...
function CategoryAccordionTrigger({ categoryKey, pieceSummary, expanded, panelId, onToggle }) {
    const style = getInventoryCategoryStyle(categoryKey);
    const Icon = style.Icon;
    return (_jsxs("button", { type: "button", className: `flex w-full items-center gap-3 px-4 py-3.5 text-left transition-colors duration-200 ${style.headerBg} ${style.headerHover}`, onClick: onToggle, "aria-expanded": expanded, "aria-controls": panelId, children: [_jsx("span", { className: `flex h-10 w-10 shrink-0 items-center justify-center rounded-xl border ${style.chipBorder} ${style.chipBg}`, "aria-hidden": true, children: _jsx(Icon, { className: `h-[1.125rem] w-[1.125rem] ${style.accentIcon}`, strokeWidth: 2 }) }), _jsx("span", { className: `inline-flex max-w-[min(100%,14rem)] shrink-0 truncate rounded-full border px-2.5 py-1 text-xs font-semibold ${style.chipBg} ${style.chipBorder} ${style.chipText}`, children: style.label }), _jsx("span", { className: `min-w-0 flex-1 text-sm tabular-nums ${style.accentText}`, children: pieceSummary }), _jsx(ChevronCategory, { open: expanded, className: style.accentIcon })] }));
}
