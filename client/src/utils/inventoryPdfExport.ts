...
export function listedInInventoryRow(part: Part): boolean {
...
export type InventoryPdfScope = "ALL" | "CATEGORY" | "LOW_STOCK" | "PREBUILT_ONLY";

export function shelfInventoryTotals(parts: Part[]): {
  units: number;
...
export function getListedInventoryParts(parts: Part[]): Part[] {
  return parts.filter(listedInInventoryRow);
}

export function filterPartsForInventoryPdf(
  listedParts: Part[],
  ...
  if (scope === "LOW_STOCK") {
    const totals = new Map<PartCategory, number>();
    for (const p of listedParts) {
      if (p.inventoryKind !== "PART" || !p.category || isNonStockCategory(p.category)) continue;
      totals.set(p.category, (totals.get(p.category) ?? 0) + p.stock);
    }
    const lowCats = new Set<PartCategory>();
    for (const [cat, sum] of totals) {
      if (sum < LOW_STOCK_CATEGORY_THRESHOLD) lowCats.add(cat);
    }
    ...
    return listedParts.filter((p) => {
      if (isPrebuiltPc(p)) return prebuiltLow;
      if (p.inventoryKind !== "PART" || !p.category || isNonStockCategory(p.category)) return false;
...
export type InventoryPdfCategoryGroup = {
  key: string;
  label: string;
  parts: Part[];
};

export function groupInventoryPartsForPdf(parts: Part[]): InventoryPdfCategoryGroup[] {
  const byCat = new Map<string, Part[]>();
  const prebuilts: Part[] = [];
  ...
  const sortByName = (a: Part, b: Part) => a.name.localeCompare(b.name, "es", { sensitivity: "base" });
  const groups: InventoryPdfCategoryGroup[] = [];
...
export function inventoryPdfScopeLabel(
  scope: InventoryPdfScope,
  category: PartCategory | null
): string {
  if (scope === "ALL") return "Inventario completo";
  if (scope === "PREBUILT_ONLY") return "Solo PCs completos (premontados)";
  if (scope === "LOW_STOCK")
    return `Stock bajo (categorías físicas con menos de ${LOW_STOCK_CATEGORY_THRESHOLD} uds. totales; PCs si el total es bajo)`;
  if (scope === "CATEGORY" && category) return `Solo categoría: ${partCategoryLabel(category)}`;
