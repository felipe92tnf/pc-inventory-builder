...
function spareRowsFromService(service: ServiceRow) {
  if (service.sparePartLines?.length) return service.sparePartLines;
...
function appendSpareLines(rows: ServicePdfLineRow[], service: ServiceRow, piecesSale: number) {
  const spareRows = spareRowsFromService(service);
  if (spareRows.length === 0 || piecesSale <= 0) return;
...
export function serviceConceptLinesForPdf(service: ServiceRow): ServicePdfLineRow[] {
  const rows: ServicePdfLineRow[] = [];
