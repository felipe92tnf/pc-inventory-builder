...
import type { CreateCatalogPayload, PartCatalogEntry } from "../types/part";
 ...
 */
export function listCatalogParts(search?: string) {
  if (import.meta.env.DEV && !loggedApiBaseOnce) {
    loggedApiBaseOnce = true;
    console.log(
      "[SecondByte catalog] URL base (VITE_API_URL):",
      import.meta.env.VITE_API_URL ?? "(no definida → http://localhost:4000/api/v1)"
    );
  }
  ...
  return http<PartCatalogEntry[]>(path)
    .then((rows) => {
      if (import.meta.env.DEV) {
...
export function listCatalog(query?: string) {
  return listCatalogParts(query);
}

export function createCatalogPart(payload: CreateCatalogPayload) {
  return http<PartCatalogEntry>("/catalog", {
    method: "POST",
    body: payload
  });
}
